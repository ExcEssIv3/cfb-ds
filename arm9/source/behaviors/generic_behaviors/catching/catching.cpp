...
bool Catching::catchBall(Player *self, const GameContext &ctx)
{
    ...
    if (ctx.football->height > self->jumpHeight + self->stats.catchRadius) return false;

    float odds = baseCatchOdds + catchSkillWeight * (self->stats.catching / 100.0f - 0.5f);
    bool caught = (rand() % 100) < (int)(odds * 100);
    char buf[64];
    snprintf(buf, sizeof(buf), "CATCH: odds=%.0f%% height=%.1f reach=%.1f %s\n",
        odds * 100, ctx.football->height, self->jumpHeight + self->stats.catchRadius,
