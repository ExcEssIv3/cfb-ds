...
- **PlayerStats** — plain struct in `player_stats.h` holding `width`, `height` (pixels), `acceleration`, `topSpeed`, `catchRadius`, `weight`, `breakTackle`, `tackle`, `block`, `blockShed`, `throwStrength`, `jump` (all default 0). Passed by value to the `Player` constructor. All players carry all stats; position-appropriate defaults are set at construction in `Roster`. Catch/intercept requires `football->height <= jump + catchRadius`.
