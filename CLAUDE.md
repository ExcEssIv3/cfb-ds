...
└── arm9/
    ...
    └── source/
        ...
        │   │   └── catching/         # Catching::catchBall(self, ctx) — shared catch/intercept logic with FAILED_CATCH gating
        │   ├── offensive_behaviors/
        ...
        └── players/
            ├── player_stats.h        # PlayerStats — width, height, acceleration, topSpeed, catchRadius, weight, breakTackle, tackle, block, blockShed, throwStrength, jump, catching (all default 0)
...
- **Player** — inherits `StatusMixin`. Has `Vector2 pos`, `Vector2 velocity`, `bool isOffense`, `Position position`, `PlayerStats stats`, `Behavior* behavior`, `int stumbleFrames`, `int jumpFrame`, `float jumpHeight`. Movement: `accelerate(direction)` steps velocity toward `topSpeed` in the given direction by `acceleration` per frame; `decelerate()` steps velocity toward zero; `move()` applies velocity to pos (`pos += velocity`). `goTo(Vector2)` uses braking distance (`v²/2a`) to decide whether to accelerate or decelerate, snapping when within `acceleration` of target. `runAI()` checks `STUMBLED` first, then `JUMPING` (advances `jumpFrame`, computes `jumpHeight` as a parabolic arc peaking at `stats.jump`, calls `move()` and returns early — no behavior or direction change while airborne), then handles fumble pickup, then delegates to `behavior->update(this, ctx)`. `startJump()` sets `JUMPING` and resets `jumpFrame`/`jumpHeight`. `JUMP_FRAMES = 30`. `detectCollision(colliderPos, width, height)` does AABB overlap between self's box and the collider's box. `Player::Status`: `BALL_CARRIER` (bit 0), `STUMBLED` (bit 1), `PANCAKED` (bit 2), `BLOCKING` (bit 3), `FAILED_CATCH` (bit 4), `JUMPING` (bit 5). No subclasses — all players are instantiated directly as `Player` with the appropriate `isOffense`, `Position`, and `PlayerStats`. `Roster` holds separate `Player* offense[]` and `Player* defense[]` arrays; `PassCatcher::player` is also `Player*`.
- **PlayerStats** — plain struct in `player_stats.h` holding `width`, `height` (pixels), `acceleration`, `topSpeed`, `catchRadius`, `weight`, `breakTackle`, `tackle`, `block`, `blockShed`, `throwStrength`, `jump` (int, max jump height in pixels), `catching` (all default 0). Passed by value to the `Player` constructor. All players carry all stats; position-appropriate defaults are set at construction in `Roster`. Catch/intercept gated on `football->height <= player->jumpHeight + catchRadius` (using the live jump height, not the static stat) and proximity, then probabilistic based on `catching` stat via `Catching::catchBall`.
...
- **Football** — inherits `StatusMixin`. `Football::Status`: `HIDDEN` (bit 0), `FLYING` (bit 1), `FUMBLED` (bit 2) — mutually exclusive, set one at a time. Uses `Vector2 pos`, `start`, and `destination`. FLYING computes `height` each frame as a normalized parabolic arc (`arcFactor * d * (D-d) / D`) in screen pixels, increments `whitePosition` (loops 0–11, 1 row per 4 frames). Does not draw directly — `Field::draw()` subtracts `height` from the Y draw position and calls `Renderer::drawFootball` passing `whitePosition`.
