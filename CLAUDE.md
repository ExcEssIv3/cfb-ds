...
- **GameContext** — `game_context.h` defines `ManDefender { defender, receiver }`, `ZoneDefender { defender }`, `DefensivePlayContext { manDefenders[5], manDefenderCount, zoneDefenders[5], zoneDefenderCount }`, and `GameContext { football, ballCarrier, playerCount, lineOfScrimmage, firstDown, offensePlay, defensePlay }`. Built each frame in `Field::update()` from `PlayContext` state and passed to all behavior `update()` calls.
